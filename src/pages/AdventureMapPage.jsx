...
export default function AdventureMapPage() {
  ...
  const activeJourney = journeyParam === 'esp32' ? 'esp32' : 'arduino'
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
              ...
              <div style={{ display: 'flex', background: 'rgba(15,23,42,0.06)', borderRadius: 10, padding: 2, gap: 1 }}>
                {['arduino', 'esp32'].map(j => (
                  <button
                    key={j}
                    onClick={() => navigate(`/adventure?journey=${j}${classId ? `&classId=${classId}` : ''}`)}
                    style={{
                      padding: '3px 10px', borderRadius: 8, border: 'none', cursor: 'pointer',
                      background: activeJourney === j ? 'rgba(255,255,255,0.7)' : 'transparent',
                      color: activeJourney === j ? '#4f46e5' : '#64748b',
                      fontWeight: 800, fontSize: 10, transition: 'all 0.2s',
                      boxShadow: activeJourney === j ? '0 1px 4px rgba(0,0,0,0.05)' : 'none',
                    }}
                  >
                    {j === 'arduino' ? 'Arduino' : 'ESP32'}
                  </button>
                ))}
              </div>
              ...
              {['arduino', 'esp32'].map(j => (
                <button
                  ...
                  style={{
                    ...
                    boxShadow: activeJourney === j ? '0 2px 6px rgba(0,0,0,0.05)' : 'none',
                  }}
                >
                  {j === 'arduino' ? 'Arduino' : 'ESP32'}
                </button>
