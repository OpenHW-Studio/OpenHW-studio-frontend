...
const load = async () => {
      ...
      const storedCriteria = localStorage.getItem("bankProjectCriteria");

      if (bankMode) {
        if (isNewBankProject) {
          const importJustRan = importedFromSimulatorRef.current;
        ...
        try {
          ...
          setAssessmentNodeContent((prev) => {
            // When returning from simulator, process criteria from localStorage
              ...
              try {
                ...
                if (parsed && Array.isArray(parsed.components) && Array.isArray(parsed.connections)) {
                  ...
                  return {
                    ...
                    autoGeneratedError: "",
                    refCircuitName: prev.refCircuitName || "Imported from simulator",
                  };
                }
              } catch (e) {
      ...
      try {
        ...
        if (!project) {
          ...
          setRewardComponents([]);
          setAssessmentNodeContent((prev) => {
            if (importJustRan && prev?.evaluationCriteria) return prev;
            return {
              passingThreshold: 60,
              refCircuitFile: null,
              refCircuitName: "",
...
const handleReturnFromSimulator = useCallback(() => {
    ...
    if (stored) {
      localStorage.removeItem("bankProjectCriteria");
    }
  }, []);
...
<button
                   ...
                   onClick={() => {
                     ...
                     navigate(`/simulator?returnTo=${encodeURIComponent("/teacher/project-bank/" + projectSlug + "/edit?simulatorCriteria=1")}`);
                   }}
                   style={{
                    padding: "10px 14px",
                    borderRadius: 10,
