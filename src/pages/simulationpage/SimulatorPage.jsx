...
// -- Gamification --
  ...
  // Always enforce unlocks — free simulator also respects adventure-map progress
  const enforcePaletteUnlocks = true;
  ...
  useEffect(() => {
    if (!activeUser) {
      // Not logged in — clear adventure list; context defaults apply
    ...
    if (!classId) {
      // Free simulator or adventure mode — GamificationContext (unlockedComponentTypes)
      // is already loaded from MongoDB.  No extra API call needed.
      setAdventureUnlockedComponents([]);
      setIsUnlocksLoading(false);
      return undefined;
    }

    // Class mode — fetch unlocks from the class-adventure API
    ...
    const loadAdventureUnlocks = async () => {
      try {
        ...
        if (!cancelled) {
          ...
          setAdventureUnlockedComponents(list);
          if (list.length > 0) {
            await unlockComponentTypes(list);
          }
          setIsUnlocksLoading(false);
        }
      } catch (e) {
        console.warn('Failed to load class adventure unlocks:', e);
        if (!cancelled) {
          setAdventureUnlockedComponents([]);
          setIsUnlocksLoading(false);
        }
  ...
  }, [classId, activeUser?._id, activeUser?.id, activeUser, unlockComponentTypes]);

  const paletteUnlockedComponents = useMemo(() => {
    // Wildcard — everything unlocked
    if (unlockedComponentTypes === "*") return ["*"];
    if (Array.isArray(adventureUnlockedComponents) && adventureUnlockedComponents.includes("*")) return ["*"];

    // Context types come from GamificationContext (backed by MongoDB via unlockService)
    const contextTypes  = Array.isArray(unlockedComponentTypes)    ? unlockedComponentTypes    : [];
    // Adventure types come from the class-adventure API (class mode only)
    const adventureTypes = Array.isArray(adventureUnlockedComponents) ? adventureUnlockedComponents : [];

    // If nothing has loaded yet, return null → PalettePanel shows loading spinner
    if (contextTypes.length === 0 && adventureTypes.length === 0 && isUnlocksLoading) return null;

    return [...new Set([...contextTypes, ...adventureTypes])];
  }, [
