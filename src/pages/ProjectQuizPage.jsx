...
export default function ProjectQuizPage() {
  ...
  const flashcardsAll = classQuizQuestions
  ...
  // Apply teacher-set quiz difficulty (slice questions)
  const quizDifficulty = (() => { try { return localStorage.getItem('openhw_quiz_difficulty') || 'all' } catch { return 'all' } })()
  const flashcards = (() => {
    if (quizDifficulty === 'beginner')     return flashcardsAll.slice(0, Math.max(2, Math.ceil(flashcardsAll.length * 0.4)))
    if (quizDifficulty === 'intermediate') return flashcardsAll.slice(0, Math.max(3, Math.ceil(flashcardsAll.length * 0.6)))
    return flashcardsAll
  })()
